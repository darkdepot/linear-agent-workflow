...
> in `plans/README.md`.
>
> **Drift check (run first)**: `git diff --stat 889742b..HEAD -- skills/linear-preflight/SKILL.md skills/linear-ship/SKILL.md skills/linear-deploy/SKILL.md skills/linear-idea/SKILL.md skills/linear-implement/SKILL.md references/artifact-quality.md references/human-friendly-output.md scripts/validate-workflow.mjs`
...
problem affects linear-idea's mandated blocked message — the first skill a
...
- `skills/linear-preflight/SKILL.md:65` — the certificate is posted to Linear:
  `Record the full preflight certificate as a Linear comment or resource with the stable marker `linear-preflight certificate``.
...
- `skills/linear-ship/SKILL.md:98-116` — "Green certificate shape": English
  block starting `linear-ship green certificate / Ship: green / …`. Recovery:
  `linear-deploy` reads it from Linear comments (skills/linear-deploy/SKILL.md:29).
- `skills/linear-deploy/SKILL.md:53-70` — "Deploy closeout shape": English
...
- `skills/linear-idea/SKILL.md:89-96` — mandated verbatim English blocked
  message ("BLOCKED / INCOMPLETE - linear-idea cannot complete because… Exit
  Plan Mode or rerun linear-idea in execution/default mode…"). The repo's own
  ...
  `linear-check` greps for the literal marker `BLOCKED / INCOMPLETE`
  (skills/linear-check/SKILL.md:63), so that token must survive.
- `skills/linear-implement/SKILL.md:100-111` — the Russian implementation-start
...
- Validator pins (verified in `scripts/validate-workflow.mjs`): the preflight
  ...
  recovery sentences at 507-516; `Requires `linear-ship green certificate``
...
**In scope**:
- `skills/linear-preflight/SKILL.md`, `skills/linear-ship/SKILL.md`,
  `skills/linear-deploy/SKILL.md`, `skills/linear-idea/SKILL.md`,
...
- Every such comment MUST open with 1-2 Russian sentences (project config
  ...
  `Ветка готова к PR: автоматическое ревью чистое, локальные проверки прошли. Дальше — linear-ship.` —
...
a. `skills/linear-preflight/SKILL.md`: in the "Human Linear comment/resource
   ...
   never translated.
b. `skills/linear-ship/SKILL.md`: same treatment for the "Green certificate
   shape" when recorded as a Linear comment (add the human-lead lines above
   `linear-ship green certificate` in the comment context; the in-chat shape
   is plan 006's concern).
c. `skills/linear-deploy/SKILL.md`: same for "Deploy closeout shape", plus
...
### Step 3: Russify the linear-idea blocked message

In `skills/linear-idea/SKILL.md:89-96`, keep the first line exactly
...
(linear-check greps the marker), and replace the remaining lines with the
contract's Russian blocked-shape: what is durable, the exact unblock step in
operator terms (`Выйди из Plan Mode (или перезапусти /linear-idea в обычном режиме) — я создам Project в статусе Idea.`),
and the no-artifacts boundary said once in Russian.

**Verify**: `grep -n "BLOCKED / INCOMPLETE - linear-idea" skills/linear-idea/SKILL.md` → 1 hit (marker intact).
**Verify**: `node scripts/validate-workflow.mjs` → exit 0.

### Step 4: Trim auditor-speak from the implementation-start comment

In `skills/linear-implement/SKILL.md:100-111`, replace the line
`Источник правды: Linear package, не raw discovery chat и не local scratch docs.`
...
**Verify**: `grep -c "Источник правды" skills/linear-implement/SKILL.md` → 0.
**Verify**: `node scripts/validate-workflow.mjs` → exit 0.
...
**Verify**: `node scripts/lint-linear-artifacts.mjs` → exit 0 and
...
- [ ] linear-idea blocked message: English marker line + Russian body
- [ ] `grep -c "Источник правды" skills/linear-implement/SKILL.md` → 0
