...
- `skills/mono-implement/SKILL.md:59` — `Record a human Linear comment that
...
- `skills/mono-preflight/SKILL.md:88` — certificate `Next:` can be
...
- `skills/mono-ship/SKILL.md:41` — a certificate is recorded only `when the
...
- `skills/mono-idea/SKILL.md:57` — the `/office-hours` vs `/brainstorming`
...
**In scope**:
- `skills/mono-implement/SKILL.md`, `skills/mono-preflight/SKILL.md`,
  `skills/mono-ship/SKILL.md`, `skills/mono-deploy/SKILL.md`,
...
- Branch: current worktree branch, or `advisor/008-mono-traces-and-tiny`.
...
a. `skills/mono-implement/SKILL.md`: in the `exit` state, require the
...
b. `skills/mono-preflight/SKILL.md`: add an ADDITIVE certificate field
...
c. `skills/mono-ship/SKILL.md`: require the exit comment for
...
d. `skills/mono-idea/SKILL.md`: require the final Project comment to include
...
- [ ] `grep -n "Tiny Output Profile" references/readiness-gates.md skills/mono-implement/SKILL.md skills/mono-preflight/SKILL.md skills/mono-ship/SKILL.md skills/mono-deploy/SKILL.md` → 5 hits
...
- The tiny profile's combined start+certificate comment would break
  mono-ship's marker-based certificate recovery — the marker line must
