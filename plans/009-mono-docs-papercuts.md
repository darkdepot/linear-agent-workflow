...
> in `plans/README.md`.
>
> **Drift check (run first)**: `git diff --stat 889742b..HEAD -- templates/prd.md templates/tech-spec.md templates/issue.md skills/mono-prd/SKILL.md scripts/validate-workflow.mjs scripts/lint-mono-artifacts.mjs examples/`
...
- `templates/prd.md:50` — acceptance-example format:
  ...
  sentence is duplicated in `skills/mono-prd/SKILL.md:34` (keep in sync).
...
- **Validator pins** (verified in `scripts/validate-workflow.mjs`):
  ...
  - **Renaming any pinned heading requires updating the pin in the same
    change.**
- **Lint pins** (`scripts/lint-mono-artifacts.mjs`): mono-prd must include
...
- `skills/mono-prd/SKILL.md` (the duplicated AE sentence + section list refs)
...
- Branch: current worktree branch, or `advisor/009-mono-docs-papercuts`.
...
`skills/mono-prd/SKILL.md` and `references/artifact-quality.md` may
reference them; `scripts/lint-mono-artifacts.mjs` does NOT pin these).
...
duplicated sentence in `skills/mono-prd/SKILL.md:34` identically. In
...
Keep `actor -> capability -> benefit` (lint-pinned) intact in mono-prd.

**Verify**: `node scripts/lint-mono-artifacts.mjs` → exit 0.
**Verify**: `node scripts/validate-workflow.mjs` → exit 0 — note the validator
pins the OLD AE sentence? Check: `grep -n "Дано" scripts/validate-workflow.mjs scripts/lint-mono-artifacts.mjs` —
...
(mono-check parses the literals `AFK`/`HITL`).

**Verify**: `node scripts/validate-workflow.mjs` → exit 0 (pin `# Готовность агента` untouched; lint pins `` `AFK` ``/`` `HITL` `` in mono-issue/SKILL.md untouched).

### Step 5: Full suite

**Verify**: `node scripts/validate-workflow.mjs` → exit 0;
`node scripts/lint-mono-artifacts.mjs` → exit 0. If plan 001 landed:
...
- [ ] AE/R slug rule present in prd template + mono-prd skill (identical sentences)
...
- A heading you are renaming turns out to be pinned in
  `scripts/lint-mono-artifacts.mjs` (it should not be at `889742b`) — stop,
...
- Existing Linear documents in real projects use the old headings and
  mono-check starts flagging them — mono-check judges "shape by semantics
...
- The old headings may still exist in real Linear docs created before this
  change; mono-check's semantics-first rule tolerates them. New documents
