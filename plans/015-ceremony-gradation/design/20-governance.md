...
- **Durable/cross-unit decision** (outlives or constrains multiple Issues/Projects) → **ADR** in the repo (GitHub), via `mono-ship` `document-pr` + `domain-modeling` (contract D §2.4). Not a forced Linear Project.
- **Local contested decision** scoped to one unit → a **structured approval comment** on the Issue/Project/Initiative (Linear), machine block under marker `mono-decision-record`, most-recent-`decision_revision` wins (route-record discipline; never a durable body — artifact-rules.md:40).
...
|---|---|---|---|---|---|---|
| Initiative + brief | `mono-initiative` (NEW, C§VI) | single writer / `mono-initiative` | `mono-review initiative` | **owner** (objective metric) | objective change; child-topology edit | child Projects; deploy cutover |
| Project + brief | `mono-project` | single writer | `mono-review handoff` | owner (package approval) | scope change | PRD/Spec/Issues |
| PRD | `mono-prd`/`mono-handoff` | single writer | `mono-review` | owner | discovery/scope change | Spec, Issue, deploy oracle |
| Tech Spec | `mono-spec`/`mono-handoff` | single writer | `mono-review`/preflight | orchestrator (technical) | scope/contract drift; stale sample | Issue, preflight |
| Issue (delivery) | `mono-issue`/`mono-issue-intake` (NEW, A§0) | single writer | preflight/ship review | owner (impl-start approval, fingerprint) | fingerprint drift | implement→deploy |
| Issue-contained safety modules (D§2) | `mono-issue`/handoff | single writer | preflight + Security lens | orchestrator (technical) unless irreversible→owner | stale migration assumptions | preflight/ship/deploy |
...
| Safety certificate (B§3) | `mono-deploy` | single writer | n/a (binds route-record) | orchestrator arm; owner if no-checkpoint/irreversible | new head_sha; missing checkpoint | deploy gate; incident recovery |
| Research certificate (D§1.5) | `mono-implement` (research mode) | single writer | `mono-check research-closeout` | owner if conclusion feeds a floor-level unit | stale invalidation keys | Tech Spec/PRD/ADR/estimate |
| Cutover group + emergent vector (C§II) | `mono-initiative`/handoff (declared) | single writer | `mono-review initiative` | owner (group deploy-approval per INV-2) | membership edit; child re-classification | `mono-deploy` group release |
...
| Containment authority (B§4) | `mono-deploy` at approval | single writer | n/a | owner grants at deploy-approval | new head_sha | incident Phase A/C |

Invariants preserved: single Linear writer during orchestration; control-plane boundary (orchestrator sequences/routes, never authors stage artifacts — the NEW `mono-initiative`/`mono-issue-intake` are atomic artifact skills, not orchestrator absorption); `mono-review` report-only; `mono-check` readiness-only.
...
5. **Cutover-group membership authority (C open-Q1)** — ADOPT C's proposal: **declared** by owner/handoff, validated by `mono-review initiative`; the reducer auto-SUGGESTS a group when members share a hard-trigger, but suggestion ≠ authority.
