...
- `prerequisites.autoreviewHelper`: must be `true`; `linear-preflight` blocks if the helper is unavailable. The workflow passes an explicit GPT-5.6 model and effort from `references/autoreview-routing.md` instead of inheriting the external helper's default.
