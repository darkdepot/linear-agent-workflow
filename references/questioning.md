...
Question stages (worker note: under `mono-orchestrate` dispatch, the
...
- `mono-idea`: ask 1-3 direction-shaping questions (outcome, boundary, audience) before creating or updating the Idea Project; resolve the rest as explicit assumptions in the brief.
- `mono-handoff`: ask only for package approval, unresolved product decisions, or accepted review fixes.
- `mono-review`: ask nothing by default; return findings and options. If a required artifact is unavailable, return `blocked`.
...
- `mono-orchestrate`: ask only for Always-ask escalations (scope, design/UX, product risk, deploy approval per the configured policy); answer worker technical questions autonomously and record them under «Решил сам:»; batch design/UX escalations into the UX checkpoint with a prepared prototype per Director Discovery in references/orchestration.md.
- `mono-preflight`: ask only when local readiness cannot proceed because of dirty-worktree ownership, material drift, missing verification access, or commit/branch risk.
- `mono-ship`: ask only when feedback requires product, UX, business, scope, or risk acceptance decisions.
- `mono-deploy`: ask only for deploy approval per the configured deploy-approval policy, or a delivery-policy/risk-acceptance decision.
