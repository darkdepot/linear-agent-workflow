...
2. Detect the latest PR head SHA before each wait, documentation run, or resolver run.
3. If a Documentation workflow is configured and has not run for the current PR head, invoke it before final review stabilization.
4. If the Documentation workflow pushes commits, record the new head SHA and restart the review loop from that head.
5. Poll relevant checks, GitHub review state, unresolved threads, and Greptile state every 10 minutes until a terminal state or timeout.
6. Detect Greptile status when available:
...
7. Inspect unresolved GitHub review threads and actionable Greptile comments when available.
8. If no actionable unresolved feedback remains, start the quiet period.
9. If actionable unresolved feedback exists and a review feedback workflow is configured, invoke it.
10. If actionable unresolved feedback exists and no review feedback workflow is configured, return `needs-human` with the unresolved feedback summary and missing resolver note.
11. If the resolver pushes commits, record the new head SHA and restart the documentation/review cycle.
12. Repeat until an exit condition is reached.
