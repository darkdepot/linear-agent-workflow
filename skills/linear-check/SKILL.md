...
- `idea` (issue-only route): PASS when the request was correctly routed to the `linear-issue-intake` front door as unmistakable one-PR issue-only work and no Project was created by design; the missing Project is expected, not a FAIL. Ambiguous or project-shaped ideas still require a Project by the rules above.
...
- `linear-idea` ended without a Linear Project link, unless it routed unmistakable one-PR issue-only work to `linear-issue-intake` (the issue-only front door creates no Project by design).
