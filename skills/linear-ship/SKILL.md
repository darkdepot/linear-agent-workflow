...
4. `create-pr`: delegate actual PR creation to `consumer.shipWorkflow`.
...
6. `stabilize-review`: when `consumer.reviewFeedbackWorkflow` is configured, run the feedback loop in `references/ship-feedback-loop.md`.
7. `land-deploy`: when the review loop is green and `consumer.landWorkflow` is configured, delegate merge/deploy to that workflow.
...
- If `consumer.reviewFeedbackWorkflow` is absent, stop after PR creation/status sync with final verdict `pr-created`.
- If `consumer.landWorkflow` is absent, stop after review stabilization with final verdict `green`.
