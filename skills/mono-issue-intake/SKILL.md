...
8. Do not activate in Phase-1 — terminate at a prepared, non-startable package. Drafting, review, fingerprinting, approval, read-back, the marker and label, and the readiness check are complete, but activation into a startable state is deferred: the downstream delivery stages (`mono-implement`, `mono-preflight`, `mono-ship`, `mono-deploy`, and orchestration) cannot yet consume a projectless package until Phase-1 Slices 3-5 (MONO-16..MONO-18). Leave the Issue non-startable and record that it is prepared and approved but not yet activatable; for work that must ship now, use Project-first. Activation — and the resolve seam the downstream stages read (the installer-published resolver at `../.mono-agent-workflow/scripts/resolve-issue-context.mjs`), `node <resolver> --issue <path> --marker <marker-comment> --config <project-config> --label issue-only --approval-verified <fingerprint>` (the marker lives in a Linear comment, so `--marker` is required — without it the resolver reads the Issue body, finds no marker, and returns project-first) (which grants issue-only only when the project config opts the lane in, the marker is valid and fresh, and the label and approval all agree, and otherwise fails closed to project-first) — goes live with the delivery slices. Keeping the package non-startable makes the staging boundary an executable gate, not a promise the workflow cannot yet honor.
