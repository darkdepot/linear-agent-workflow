...
- Discovery: Project plus discovery outputs; durable PRD/Tech Spec may exist and are normally created by `mono-handoff`.
...
- Delivery: `mono-implement` has verified Project plus PRD plus Tech Spec or explicit no-spec exception plus approved execution Issue(s), explicit implementation-start approval, and readiness to begin from those Issue(s).
- Ship: Issue and PR state are synced by `mono-ship`, not by Project Updates.

Before finishing:

- Run or report `mono-check <mode>` for the transition you are supporting.
