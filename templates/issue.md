...
- `Готовность агента` must say `AFK` or `HITL`. Use `AFK` only when a zero-context implementation agent can execute without new human judgment. Use `HITL` when product, design, external access, manual QA, or risk acceptance is still required, and name the exact dependency.
